...
function NoirLaw() {
        const [mydata, setMyData] = useState([]);
            ...
            useEffect(() => {
                ...
                try {
                  const response = await fetch('http://localhost:8080/NoirX/lawyers');
           ...
           return(
            <>
                <div
                  className="bg-cover bg-amber-900 h-screen overflow-auto">
                  <Navbar/>
                  <div className="header"><h1 className='ml-6 mt-4 text-white text-center bold font-bold text-9xl'>NoireX Law</h1></div>
            ...
            {mydata.map((item, index) => {
              return (
                <>
                 ...
                 <div key={index} >
                 <div className="flex flex-col items-center m-5 p-2.5 border-4 rounded-3xl rounded-tl border-black w-[400px] h-[320px] bg-orange-600 bg-opacity-80">
      <img src={item.image_Url} alt="item" className="w-full h-full object-contain" />
                 ...
                 <div className='gallery-description'><div className='text-xl text-white'>
                  ...
                  <p style={{display: 'flex', justifyContent:'center', margin: 'auto'}}>{item.practice}</p>
