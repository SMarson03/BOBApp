...
function NoirTherapy() {
    ...
    useEffect(() => {
        ...
        try {
          const response = await fetch('http://localhost:8080/NoirX/therapists');
        ...
        {mydata.map((item, index) => {
          return (
            <>
             ...
             <div key={index} >
             <div className="flex flex-col items-center m-5 p-2.5 border-4 rounded-3xl rounded-tl border-black w-[400px] h-[320px] bg-black bg-opacity-80">
  <img src={item.image_Url} alt="item" className="w-full h-full object-contain" />
             ...
             <div className='gallery-description'><div className='text-xl text-white'>
              ...
              <p style={{display: 'flex', justifyContent:'center', margin: 'auto'}}>{item.certified}</p>
